...
QUESTION = (
    "A software vendor breached a $250,000 implementation contract and the buyer suffered "
    "foreseeable consequential losses. What remedies, damages, and relevant case law apply?"
)
...
async def main():
    ...
    llm = get_llm()
    agent_kwargs = {"model": llm, "tools": TOOLS, "prompt": SYSTEM_PROMPT}
    agent_parameters = inspect.signature(create_react_agent).parameters
    if "verbose" in agent_parameters:
        agent_kwargs["verbose"] = True
    elif "debug" in agent_parameters:
